...
- If current last/mark/index price, 1H/4H candles, funding, OI, or major event status is missing, do not give a high-confidence directional trade; use `no trade`, `trigger long`, or `trigger short`.
...
- Core data conflict or unconfirmed news as main catalyst: no directional main call; use explicit triggers.
